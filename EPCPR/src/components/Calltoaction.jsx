import React from 'react'

const Calltoaction = () => {
  return (
    <div className="px-4 md:px-8 lg:px-20 py-8 md:py-12 lg:py-20">
      <h1 className="font-extrabold text-xl md:text-3xl lg:text-4xl text-center">
        Do you want to boost your growth with PR?
      </h1>
      <p className="pt-3 px-4 md:px-6 text-sm md:text-base lg:text-lg text-center max-w-3xl mx-auto">
        We've helped +100 clients achieve their business goals with the power of Strategic PR across key markets.
        <br className="hidden md:block" /> 
        All our PR strategies are custom-made to fit your needs and meet you at the stage of growth you're at.
      </p>
      <div className="flex justify-center mt-6 md:mt-8 lg:mt-10">
        <button className="btn btn-primary w-44 sm:w-auto">Book a service</button>
      </div>
    </div>
  )
}

export default Calltoaction